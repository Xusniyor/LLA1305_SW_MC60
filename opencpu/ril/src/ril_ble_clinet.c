...
s32 RIL_BT_Gatcgc(ST_BLE_Client *gclie)
{
    ...
    {
        Ql_sprintf(strAT, "AT+QBTGATCGC=\"%s\",%d,\"%s\",%d,%d", gclie->gclient_id,
                   gclie->conn_status.connect_id, gclie->service_id[sid].service_uuid,
                   gclie->service_id[sid].service_inst, gclie->service_id[sid].is_primary);
    }
    else
    {
        cid = cid - 1;
        Ql_sprintf(strAT, "AT+QBTGATCGC=\"%s\",%d,\"%s\",%d,%d,\"%s\",%d", gclie->gclient_id,
                   gclie->conn_status.connect_id, gclie->service_id[sid].service_uuid,
                   gclie->service_id[sid].service_inst, gclie->service_id[sid].is_primary,
                   gclie->service_id[sid].char_id[cid].char_uuid, gclie->service_id[gclie->sid].char_id[cid].char_inst);
...
s32 RIL_BT_Gatcgd(ST_BLE_Client *gclie)
{
    ...
    if (0 == did)
    {
        Ql_sprintf(strAT, "AT+QBTGATCGD=\"%s\",%d,\"%s\",%d,%d,\"%s\",%d", gclie->gclient_id,
                   gclie->conn_status.connect_id, gclie->service_id[sid].service_uuid,
                   gclie->service_id[sid].service_inst, gclie->service_id[sid].is_primary,
                   gclie->service_id[sid].char_id[cid].char_uuid, gclie->service_id[sid].char_id[cid].char_inst);
    }
    else
    {
        Ql_sprintf(strAT, "AT+QBTGATCGD=\"%s\",%d,\"%s\",%d,%d,\"%s\",%d,\"%s\",%d", gclie->gclient_id,
                   gclie->conn_status.connect_id, gclie->service_id[sid].service_uuid, gclie->service_id[sid].service_inst,
                   gclie->service_id[sid].is_primary, gclie->service_id[sid].char_id[cid].char_uuid,
                   gclie->service_id[sid].char_id[cid].char_inst, gclie->service_id[sid].char_id[cid].desc_id[did - 1].desc_uuid,
                   gclie->service_id[sid].char_id[cid].desc_id[did - 1].inst);
    }
    ret = Ql_RIL_SendATCmd(strAT, Ql_strlen(strAT), NULL, NULL, 0);
    (void)ret;
    return gclie->result;
...
s32 RIL_BT_Gatcrc(ST_BLE_Client *gclie, u8 auth_req)
{
    ...
    Ql_memset(strAT, 0x0, sizeof(strAT));
    cid = gclie->service_id[gclie->sid].cid;
    Ql_sprintf(strAT, "AT+QBTGATCRC=\"%s\",%d,\"%s\",%d,%d,\"%s\",%d,%d",
               gclie->gclient_id, gclie->conn_status.connect_id,
               gclie->service_id[gclie->sid].service_uuid, gclie->service_id[gclie->sid].service_inst,
               gclie->service_id[gclie->sid].is_primary, gclie->service_id[gclie->sid].char_id[cid].char_uuid,
               gclie->service_id[gclie->sid].char_id[cid].char_inst, auth_req);
...
s32 RIL_BT_Gatcwc(ST_BLE_Client *gclie, u8 write_type, u8 auth_req, u8 *char_value)
{
    ...
    Ql_sprintf(strAT, "AT+QBTGATCWC=\"%s\",%d,%d,\"%s\",%d,%d,\"%s\",%d,\"%s\",%d",
               gclie->gclient_id, gclie->conn_status.connect_id, write_type, gclie->service_id[gclie->sid].service_uuid,
               gclie->service_id[gclie->sid].service_inst, gclie->service_id[gclie->sid].is_primary,
               char_op->char_uuid, char_op->char_inst, char_value, auth_req);
...
s32 RIL_BT_Gatcwd(ST_BLE_Client *gclie, u8 write_type, u8 auth_req, u8 *char_value)
{
    ...
    Ql_sprintf(strAT, "AT+QBTGATCWD=\"%s\",%d,%d,\"%s\",%d,%d,\"%s\",%d,\"%s\",%d,\"%s\",%d",
               gclie->gclient_id, gclie->conn_status.connect_id, write_type,
               gclie->service_id[gclie->sid].service_uuid, gclie->service_id[gclie->sid].service_inst,
               gclie->service_id[gclie->sid].is_primary, gclie->service_id[gclie->sid].char_id[cid].char_uuid,
               gclie->service_id[gclie->sid].char_id[cid].char_inst, gclie->service_id[gclie->sid].char_id[cid].desc_id[did].desc_uuid,
               gclie->service_id[gclie->sid].char_id[cid].desc_id[did].inst, char_value, auth_req);
...
void OnURCHandler_BTGatcgc(const char *strURC, void *reserved)
{
    ...
    APP_DEBUG("strURC:%s\r\n", strURC);
    Ql_strcpy((char *)urcHead, "\r\n+QBTGATCGC:");
    if (Ql_StrPrefixMatch(strURC, (const char *)urcHead))
    {
        // APP_DEBUG("%s\r\n",strURC);
        Ql_sscanf(strURC, "%*[^\"]\"%[^\"]%*[^\r\n]\r\n", clie_id);
        Ql_sscanf(strURC, "%*[^,],%*[^,],%*[^,],\"%[^\"],%*[^\r\n]\r\n", service_uuid);
        ...
        if ((callback_bt != NULL) && (Ql_strlen((const char *)cgc.char_uuid) != 0))
        {
