...
#define IS_SUPPORT_AUD_CHANNEL(audChannel)                                                                                               \
    (                                                                                                                                    \
        (                                                                                                                                \
            (AUD_CHANNEL_NORMAL == (audChannel)) || (AUD_CHANNEL_HEADSET == (audChannel)) || (AUD_CHANNEL_LOUD_SPEAKER == (audChannel))) \
            ? TRUE                                                                                                                       \
            : FALSE)
...
#define IS_SUPPORT_VOLUME_TYPE(volume_type)                                                                                                                                                                                                                      \
    (                                                                                                                                                                                                                                                            \
        (                                                                                                                                                                                                                                                        \
            (VOL_TYPE_CTN == (volume_type)) || (VOL_TYPE_GMI == (volume_type)) || (VOL_TYPE_KEY == (volume_type)) || (VOL_TYPE_MEDIA == (volume_type)) || (VOL_TYPE_MIC == (volume_type)) || (VOL_TYPE_SID == (volume_type)) || (VOL_TYPE_SPH == (volume_type))) \
            ? TRUE                                                                                                                                                                                                                                               \
            : FALSE)
...
static s32 ATResponse_AUD_handler(char *line, u32 len, void *userdata)
{
    ...
    if ((head = Ql_RIL_FindString(line, len, "+QAUDCH:")) ||
        (head = Ql_RIL_FindString(line, len, "+QMEDVL:")) ||
        (head = Ql_RIL_FindString(line, len, "+CLVL:")) ||
        (head = Ql_RIL_FindString(line, len, "+QSIDET:")) ||
        (head = Ql_RIL_FindString(line, len, "+QAUDRD:")))
    {
        char strTmp[10];
        char *p1 = NULL;
        char *p2 = NULL;
